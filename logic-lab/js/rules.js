...
if(r.rule==='→I'){if(ranges.length!==1||r.refs.length!==1)return[false,'→I는 보조증명 범위 하나(예: 2-4)를 참조합니다.'];let g=ranges[0],a=rows[g.start-1],z=rows[g.end-1],end=asts[g.end-1];if(g.start<1||g.end>=r.n||g.start>g.end||!a||!z||!end)return[false,'참조한 보조증명 범위를 확인하세요.'];if(a.rule!=='AS')return[false,'→I의 참조 범위는 AS 행에서 시작해야 합니다.'];if(!a.scope.includes(g.start)||!z.scope.includes(g.start))return[false,'참조 범위가 하나의 보조증명을 이루지 않습니다.'];if(r.scope.includes(g.start))return[false,'→I 행은 해제하려는 가정의 보조증명 밖에 있어야 합니다.'];if(cur.k!=='imp'||!E(cur.l,asts[g.start-1])||!E(cur.r,end))return[false,'현재 문장은 보조증명의 가정과 마지막 문장으로 만든 조건문이어야 합니다.'];return[true,'→I 적용이 올바르며 가정이 해제되었습니다.'];}
